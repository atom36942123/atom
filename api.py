...
async def function_database_init(request:Request):
   ...
   for k,v in config_database.items():
      for table in v[0]:
         ...
         if k not in ["alter_query"] and v[3] is not None and f'constraint_checkin_{k}_{table}' not in schema_constraint_name_list:await request.state.postgres_object.fetch_all(query=f"alter table {table} add constraint {f'constraint_checkin_{k}_{table}'} check ({k} in {v[3]});",values={})
         ...
         if k in ["alter_query"] and table.split()[5] not in schema_constraint_name_list:await request.state.postgres_object.fetch_all(query=table,values={})
