...
async def function_my_message_inbox(request:Request,page:int,is_unread:int=None,limit:int=30):
   ...
   for user_column in ["created_by_id","received_by_id"]:
      query=f"select * from users where id in ({','.join([str(item[user_column]) for item in output if user_column in item and item[user_column]])});"
      return query
      output_user=await request.state.postgres_object.fetch_all(query=,values={})
      return output_user
