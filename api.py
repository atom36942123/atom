...
async def function_database_init(request:Request):
   ...
   [await request.state.postgres_object.fetch_all(query=f"alter table {k} add constraint constraint_unique_{k}_{v} unique ({v});",values={}) for k,v in {"users":"username","likes":"created_by_id,parent_table,parent_id","bookmark":"created_by_id,parent_table,parent_id"}.items()]
