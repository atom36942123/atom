...
async def function_database_init(request:Request):
   ...
   for k,v in config_database.items():
      ...
      for table in v[0]:
         ...
         if k!="alter_query" and v[4]==1 and v[1]=="array":await request.state.postgres_object.fetch_all(query=f"create index if not exists {f'index_{k}_{table}'} on {table} using gin ({k});",values={})
