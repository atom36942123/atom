...
async def api_func(x:str,request:Request,background_tasks:BackgroundTasks):
   #token check
...
async def api_func(x:str,request:Request,action:str,table:str,ids:str):
   #token check
   response=await function_token_decode(request,config_jwt_secret_key)
   if response["status"]==0:return function_http_response(400,0,response["message"])
   request_user=response["message"]
...
async def api_func(x:str,request:Request,table:str,parent_table:str,page:int):
   #token check
   response=await function_token_decode(request,config_jwt_secret_key)
   if response["status"]==0:return function_http_response(400,0,response["message"])
   request_user=response["message"]
...
async def api_func(x:str,request:Request,page:int,is_unread:int=None):
   #token check
   response=await function_token_decode(request,config_jwt_secret_key)
   if response["status"]==0:return function_http_response(400,0,response["message"])
   request_user=response["message"]
...
async def api_func(x:str,request:Request,user_id:int,page:int,background_tasks:BackgroundTasks):
   #token check
   response=await function_token_decode(request,config_jwt_secret_key)
   if response["status"]==0:return function_http_response(400,0,response["message"])
   request_user=response["message"]
...
async def api_func(request:Request,x:str,mode:Literal["message_all","message_mutual","message_thread","like_post","bookmark_post"],user_id:int=None,post_id:int=None,message_id:int=None):
    #token check
    response=await function_token_decode(request,config_jwt_secret_key)
    if response["status"]==0:return function_http_response(400,0,response["message"])
    request_user=response["message"]
...
async def api_func(x:str,request:Request,background_tasks:BackgroundTasks):
   ...
   request_user=response["message"]
   #admin check
   if request_user["is_admin"]==1:return function_http_response(400,0,"admin user cant be deleted")
   #logic
