...
async def function_aws(request:Request):
   ...
   if body["mode"]=="ses":output=boto3.client("ses",region_name=env.list("ses")[1],aws_access_key_id=env.list("aws")[0],aws_secret_access_key=env.list("aws")[1]).send_email(Source=env.list("ses")[0],Destination={"ToAddresses":[body["email"]]},Message={"Subject":{"Charset":"UTF-8","Data":body["title"]},"Body":{"Text":{"Charset":"UTF-8","Data":body["description"]}}})
   #response
   return {"status":1,"message":output}
